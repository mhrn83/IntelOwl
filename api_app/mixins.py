    ...
    def _get_relationship_for_classification(cls, obs_clfn: str, iocs: bool) -> List:
        ...
        if obs_clfn == Classification.DOMAIN:
            ...
            raise AnalyzerRunException(
                f"Not supported observable type {obs_clfn}. Supported are: hash, ip, domain and url."
            )
...
class VirusTotalv3AnalyzerMixin(VirusTotalv3BaseMixin, BaseAnalyzerMixin, metaclass=abc.ABCMeta):
    ...
    def _vt_get_relationships(
        ...
        try:
            ...
            logger.error(
                f"something went wrong when extracting relationships for observable {observable_name}: {e}"
            )
    ...
    def _vt_scan_file(self, md5: str, rescan_instead: bool = False) -> Dict:
        ...
        if not scan_id:
            raise AnalyzerRunException(
                f"no scan_id given by VirusTotal to retrieve the results (Job: {self.job_id}, {md5})"
            )
                ...
                if last_analysis_results:
                    ...
                    if self.force_active_scan_if_old and not already_done_active_scan_because_report_was_old:
                        ...
                        if some_days_ago > scan_date_time:
                            ...
                            logger.info(
                                f"hash {observable_name} found on VT with AV reports and scan is recent"
                            )
                            break
                    else:
    ...
    def _make_request(
        ...
        try:
            if method.upper() == "POST":
                ...
                response = requests.get(url, headers=headers, json=params, timeout=60)
...
class MISPMixin(metaclass=abc.ABCMeta):
    ...
    def create_misp_event(self, base_attr: tuple) -> pymisp.MISPEvent:
        ...
        base_object = pymisp.MISPObject(
            name='domain-ip',
            strict=True
        )

        attr_value, attr_type = base_attr
        if attr_type == Classification.IP:
            base_object.add_attribute('ip', attr_value)
        else:
    ...
    def find_event_attr(self, event: pymisp.MISPEvent, attr_value: str):
        """
        Finds the uuid of the specified attribute within the event.
        """
        attr_id = next(
            (attr.uuid for attr in event.attributes if attr.value == attr_value), None)
        return attr_id

    def find_object_attr(self, event: pymisp.MISPEvent, attr_value: str, object_name: str = ""):
        """
        Finds the specified attribute and its object
        within the objects of a specific event.
        """
        if not attr_value:
            return (None, None)

        for object in event.objects:
            if object_name and object.name != object_name:
                continue

            existing_attr = next(
                (attr for attr in object.attributes if attr.value == attr_value), None)
            if existing_attr:
                return (existing_attr, object)
